...
public:
	...
	void consultar(int fila, int columna) {
	...
	void seleccionar() {
		...
		for (int i = 1; i <= filas; ++i) {
			col = datos->buscar(i);
			aux = new DF::ArbolCOL(i);
			nuevo->insertar(aux);
			for (int k = 0; k < seleccion.size(); ++k) {
				celda = col->buscar(seleccion.at(k));
				insertar = new DF::Nodo(celda->getDato(),k+1);
				aux->insertar(insertar);
			}
			aux->destruir();
		}
